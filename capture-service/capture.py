...
# Mode & file inputs
MODE = os.getenv("MODE", "PCAP")  # LIVE or PCAP
PCAP_FILE = os.getenv("PCAP_FILE", "sample-pcaps/dns.cap")

# Parser service endpoint
PARSER_URL = "http://127.0.0.1:5001/parse"

def send_packet(pkt, source="LIVE"):
    """Send packet summary + raw hex + source + timestamp to parser"""
    data = {
        "raw": pkt.summary(),
        "hex": bytes(pkt).hex(),
        "source": source,
        "timestamp": datetime.utcnow().isoformat()  # keep consistent timestamp
    }
    try:
    ...
    if MODE.upper() == "LIVE":
        ...
        try:
            ...
            try:
                ...
                print(f"❌ Could not read {PCAP_FILE}: {e}")
                packets = []
