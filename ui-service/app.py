...
def index():
    ...
    source = request.args.get("source")  # NEW filter
    packets = []
    summary = {}
    try:
        summary = requests.get(f"{ANALYZER_URL}/protocol_summary").json()
        query = f"{ANALYZER_URL}/packets"
        if source:
            query += f"?source={source}"
        if protocol:
            query += ("&" if "?" in query else "?") + f"protocol={protocol}"
        packets = requests.get(query).json()
...
def api_packets():
    """Return packets with pagination and source info"""
...
def api_filter():
    ...
    try:
        query = f"{ANALYZER_URL}/filter?protocol={protocol}"
        if source:
            query += f"&source={source}"
        packets = requests.get(query).json()
        paginated = packets[offset:offset + limit]
